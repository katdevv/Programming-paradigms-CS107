...
// Prints random text based on grammar recurively
void printGrammar(map<string, Definition>& grammar, string curr){
	// If the string is non-terminal
	if(curr[0] == '<' && curr[curr.length() - 1] == '>'){
		// Go through its productions
		Production p = grammar[curr].getRandomProduction();
        for (Production::iterator c = p.begin(); c != p.end(); ++c) {
			// Recursive step
	      	printGrammar(grammar, *c);
	    }
	}
	// If string is terminal, print
	else {
		cout << curr << " ";
	}
}
