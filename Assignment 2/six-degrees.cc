...
// Shortest path from an actor to an actor
void generateShortestPath(const string& source, const string& target, const imdb& db){
	...
	while(!partialPaths.empty() && partialPaths.front().getLength() <= 5){
		path cur = partialPaths.front(); partialPaths.pop_front();
		string lastActor = cur.getLastPlayer();
		vector<film> films;
		if(!db.getCredits(lastActor, films)){
			cout << lastActor << " doesn't have a credits." << endl;
			return;
		}
		for(film f : films){
			if (previouslySeenFilms.find(f) != previouslySeenFilms.end()) continue;
			previouslySeenFilms.insert(f);
			vector<string> actors;
			if(!db.getCast(f, actors)){
				cout << f.title << " doesn't have a cast." << endl;
				return;
			}
